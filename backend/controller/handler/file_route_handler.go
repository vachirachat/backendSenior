...
func (h *FileRouteHandler) Mount(rg *gin.RouterGroup) {
	...
	rg.POST("/room/:roomId/after-upload/:fileId", h.mw.AuthRequired("user", "add"), h.mw.IsInRoomMiddleWare("roomId"), g.InjectGin(h.afterUploadFile))
	rg.GET("/room/:roomId/files", h.mw.AuthRequired("user", "view"), h.mw.IsInRoomMiddleWare("roomId"), g.InjectGin(h.getRoomFiles))
	...
	rg.POST("/room/:roomId/after-upload-image/:fileId", h.mw.AuthRequired("user", "view"), h.mw.IsInRoomMiddleWare("roomId"), g.InjectGin(h.afterUploadImage))
	rg.GET("/room/:roomId/images", h.mw.AuthRequired("user", "view"), h.mw.IsInRoomMiddleWare("roomId"), g.InjectGin(h.getRoomImages))
