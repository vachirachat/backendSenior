...
func (userMongo UserRepositoryMongo) GetAllUserSecret() ([]model.UserLogin, error) {
	...
	err := userMongo.ConnectionDB.DB(collectionUser).C(collectionSecret).Find(nil).All(&Users)
	return Users, err
}
...
func (userMongo UserRepositoryMongo) EditUserName(userID bson.ObjectId, user model.User) error {
	...
	return userMongo.ConnectionDB.DB(collectionUser).C(collectionUser).UpdateId(userID, newName)
}
...
func (userMongo UserRepositoryMongo) AddToken(UserToken model.UserToken) error {
	return userMongo.ConnectionDB.DB(collectionUser).C(collectionToken).Insert(UserToken)
}
...
// 	return userMongo.ConnectionDB.DB(collectionUser).C(collectionToken).Update(bson.M{"Email": Email}, newToken)
// }

func (userMongo UserRepositoryMongo) GetUserTokenById(Email string) (model.UserToken, error) {
	var userToken model.UserToken
	//objectID := bson.ObjectIdHex(userID)
	err := userMongo.ConnectionDB.DB(collectionUser).C(collectionToken).Find(bson.M{"Email": Email}).One(&userToken)
...
func (userMongo UserRepositoryMongo) GetUserIdByToken(token string) (model.UserToken, error) {
	var userToken model.UserToken
	//objectID := bson.ObjectIdHex(userID)
	err := userMongo.ConnectionDB.DB(collectionUser).C(collectionToken).Find(bson.M{"Token": token}).One(&userToken)
	return userToken, err
}

func (userMongo UserRepositoryMongo) GetAllUserToken() ([]model.UserToken, error) {
	var UsersToken []model.UserToken
	err := userMongo.ConnectionDB.DB(collectionUser).C(collectionToken).Find(nil).All(&UsersToken)
...
func (userMongo UserRepositoryMongo) GetUserByEmail(email string) (model.User, error) {
	...
	err := userMongo.ConnectionDB.DB(collectionUser).C(collectionUser).Find(bson.M{"email": email}).One(&user)
	return user, err
}

// user secrect
func (userMongo UserRepositoryMongo) AddUserSecrect(user model.UserLogin) error {
	return userMongo.ConnectionDB.DB(collectionUser).C(collectionSecret).Insert(user)
}

// oauth Add token
func AddToken(UserToken model.UserToken) error {
	var ConnectionDB *mgo.Session
	return ConnectionDB.DB(collectionUser).C(collectionToken).Insert(UserToken)
}
...
func GetUserIdByToken(token string) (model.UserToken, error) {
	...
	var ConnectionDB *mgo.Session
	//objectID := bson.ObjectIdHex(userID)
	err := ConnectionDB.DB(collectionUser).C(collectionToken).Find(bson.M{"Token": token}).One(&userToken)
	return userToken, err
}
